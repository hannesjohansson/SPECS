...
**Context** — The framing material that all collaborators share: narrative, architecture, principles.
...
**Context is layered by rate of change.** Narrative changes quarterly. Architecture and blueprints change when things ship. Goals and stories evolve with understanding. Specs change weekly. Findings accumulate forever.
...
**Narrative** — What the product is, who it's for, why it exists. The durable story that outlives any feature.
...
**Discovery** — A planned investigation to reduce uncertainty. Time-boxed. Lives in `discoveries/` while in progress; emits a finding on completion and retires. No code is generated from a discovery.
...
**Skill** — A procedural recipe for a specific kind of work (drafting a spec, scoping a release, resolving drift). Lives in `skills/`. Loads only when its trigger fires.
...
Skills live in `skills/` and carry procedure for specific kinds of work. Each skill declares its trigger and scope. A skill only loads when its trigger fires, keeping session context small.
...
| Generated blueprint insufficient for coordination | Curated blueprint in `context/blueprint.md` |
| Three or more specs in one domain | Domain subfolder with its own checks |
| Standards repeating across specs | Principles file in `context/principles/` |
